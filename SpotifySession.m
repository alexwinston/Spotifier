...
//	audio_fifo_t *af = sessobj(sess).audiofifo;
    ...
    UInt32 packetsPerBuffer = state.buffer_size / state.desc.mBytesPerPacket;
    UInt32 frameCount = packetsPerBuffer;
    
    printf("%lld\n", state.currentPacket);
    
//    OSStatus status =
    AudioFileWritePackets(state.audioFile, false, frameCount, NULL, state.currentPacket / state.desc.mBytesPerPacket , &frameCount, afd->samples);
    //    if(noErr != status) {
    //        puts("AudioFileWritePackets noErr");
    state.currentPacket += frameCount * state.desc.mBytesPerPacket;
...
//	audio_init(&audiofifo);
    ...
    state.desc.mFormatFlags = kLinearPCMFormatFlagIsSignedInteger | kLinearPCMFormatFlagIsPacked | kAudioFormatFlagsNativeEndian;
    ...
    char *filename = "/Users/alexwinston/Desktop/spotify.aiff";
    CFURLRef fileURL = CFURLCreateFromFileSystemRepresentation(NULL, (UInt8 *) filename, strlen(filename), false);
    
    OSStatus status = AudioFileCreateWithURL(fileURL, kAudioFileCAFType, &state.desc, kAudioFileFlags_EraseFile, &state.audioFile);
    if (noErr != status)
