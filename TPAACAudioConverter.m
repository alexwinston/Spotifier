...
//    NSLog(@"%f", destinationFormat.mSampleRate);
...
//    NSLog(@"%f", clientFormat.mSampleRate);
...
//    }
    ...
    OSStatus err = ExtAudioFileGetProperty(destinationFile,
                                  kExtAudioFileProperty_AudioConverter, &size, &outConverter);
    if (err)
        NSLog(@"ExtAudioFileGetProperty kExtAudioFileProperty_AudioConverter, %d", err);
                       
    if (!err) {
        UInt32 prop = kAudioConverterSampleRateConverterComplexity_Mastering;
        AudioConverterSetProperty(outConverter, kAudioConverterSampleRateConverterComplexity, sizeof(prop), &prop);
        
        // Set bit rate
        SInt32 bitRate = 256000; // 320000
        err = AudioConverterSetProperty(outConverter, kAudioConverterEncodeBitRate, sizeof (bitRate), &bitRate);
        if (err != noErr)
            NSLog(@"AudioConverterSetProperty kAudioConverterEncodeBitRate, %d", err);

        CFArrayRef config = NULL;
        err = ExtAudioFileSetProperty(destinationFile, kExtAudioFileProperty_ConverterConfig, sizeof(config), &config);
        if (err)
            NSLog(@"ExtAudioFileSetProperty kExtAudioFileProperty_ConverterConfig, %d", err);
    }
...
//            }
//        } else 
        if ( !checkResult(status, "ExtAudioFileWrite") ) {
            if ( sourceFile ) ExtAudioFileDispose(sourceFile);
            ExtAudioFileDispose(destinationFile);
            [self performSelectorOnMainThread:@selector(reportErrorAndCleanup:)
                                   withObject:[NSError errorWithDomain:TPAACAudioConverterErrorDomain
